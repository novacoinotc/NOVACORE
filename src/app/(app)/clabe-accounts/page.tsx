...
export default function ClabeAccountsPage() {
  ...
  const [autoGenerateAlias, setAutoGenerateAlias] = useState('');
  ...
  // Auto-generate CLABE via OPM API
  const handleAutoGenerate = async () => {
    if (!autoGenerateAlias) {
      setAutoGenerateError('El alias es requerido');
      return;
    }
    ...
    } else if (currentUser?.role === 'super_admin' && clabeAccounts.length > 0) {
      // For super_admin, use the first company's ID or filter selection
      effectiveCompanyId = filterCompany !== 'all' ? filterCompany : clabeAccounts[0].companyId;
    }

    if (!effectiveCompanyId) {
      setAutoGenerateError('No se pudo determinar la empresa');
      return;
    }
    ...
    try {
      ...
      const response = await fetch('/api/clabe-accounts/generate', {
        ...
        body: JSON.stringify({
          ...
          description: autoGenerateDescription || undefined,
        }),
      });
      ...
      if (response.ok) {
        ...
        setAutoGenerateAlias('');
        setAutoGenerateDescription('');
      } else {
        setAutoGenerateError(data.error || data.detail || 'Error al generar CLABE');
      }
    } catch (error) {
      console.error('Auto-generate error:', error);
      setAutoGenerateError('Error al generar cuenta CLABE automáticamente');
    } finally {
      setSaving(false);
    }
  };
        ...
        {hasPermission('clabe.create') && (
          <div className="flex gap-2">
            <motion.button
              ...
              className="px-4 py-2 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white rounded-lg flex items-center gap-2 text-sm font-medium transition-all shadow-lg shadow-purple-500/25"
            >
            ...
            <motion.button
              ...
              className="px-4 py-2 bg-white/10 hover:bg-white/20 text-white rounded-lg flex items-center gap-2 text-sm font-medium transition-colors border border-white/10"
            >
        ...
        {isAutoGenerateModalOpen && (
          ...
          >
            ...
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className="relative w-full max-w-lg bg-[#0a0a1a] border border-white/[0.08] rounded-2xl shadow-2xl"
            >
              {/* Modal header */}
              <div className="border-b border-white/[0.06] px-6 py-4 flex items-center justify-between">
                <h2 className="text-lg font-semibold text-white flex items-center gap-2">
                  <Sparkles className="w-5 h-5 text-purple-400" />
                  Auto-generar Cuenta CLABE
                </h2>
                <button
                  onClick={() => setIsAutoGenerateModalOpen(false)}
                  className="p-2 text-white/40 hover:text-white hover:bg-white/[0.05] rounded-lg transition-colors"
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="p-3 bg-purple-500/10 border border-purple-500/20 rounded-lg">
                  <p className="text-purple-300 text-sm">
                    OPM generará automáticamente un número CLABE único de 18 dígitos para esta sub-cuenta.
                  </p>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    type="text"
                    value={autoGenerateAlias}
                    onChange={(e) => setAutoGenerateAlias(e.target.value)}
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    onChange={(e) => setAutoGenerateDescription(e.target.value)}
                    placeholder="Descripción adicional de la cuenta..."
                    rows={2}
                    className="w-full bg-white/[0.03] border border-white/[0.08] rounded-lg py-2.5 px-4 text-white text-sm placeholder:text-white/30 focus:outline-none focus:border-purple-500/50 transition-colors resize-none"
                  />
              ...
              <div className="border-t border-white/[0.06] px-6 py-4 flex items-center justify-end gap-3">
                ...
                <motion.button
                  ...
                  disabled={saving}
                  className="px-4 py-2 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white rounded-lg text-sm font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                >
                  {saving ? (
                  ...
                  {saving ? 'Generando CLABE...' : 'Generar CLABE'}
                </motion.button>
              </div>
            </motion.div>
          </motion.div>
