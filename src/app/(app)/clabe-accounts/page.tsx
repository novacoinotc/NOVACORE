...
const defaultFormData: ClabeFormData = {
  id: '',
  companyId: '',
  clabe: '', // Used for editing or manual entry
  alias: '',
  description: '',
  isActive: true,
...
export default function ClabeAccountsPage() {
  ...
  const handleSave = async () => {
    ...
    // For manual mode, CLABE is required
    if (!selectedClabe && formData.inputMode === 'manual' && !formData.clabe) {
      setError('La CLABE es requerida en modo manual');
      return;
    }

    // Validate CLABE format for manual entry (18 digits)
    if (!selectedClabe && formData.inputMode === 'manual') {
      const clabeRegex = /^[0-9]{18}$/;
      if (!clabeRegex.test(formData.clabe)) {
